...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Intent si = new Intent(getApplicationContext(), SoundRecordActivity.class);
		...
		si.putExtra(SoundRecordActivity.EXTRA_OUTPUT, output.getPath());
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code MIC_SOUND_REQUEST
		startActivityForResult(si, MIC_SOUND_REQUEST);
	}
	...
	private void launchCameraIntent() {
		...
		ci.putExtra(MediaStore.EXTRA_OUTPUT, fragment.imagePath);
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_PIC_REQUEST
		startActivityForResult(ci, CAMERA_PIC_REQUEST);
	...
	private void launchVideoCameraIntent() {
		...
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		ci.putExtra(MediaStore.EXTRA_OUTPUT, getOutputMediaFileUri(MEDIA_TYPE_VIDEO));
		...
		ci.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, "1");

		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_VIDEO_REQUEST
		startActivityForResult(ci, CAMERA_VIDEO_REQUEST);
